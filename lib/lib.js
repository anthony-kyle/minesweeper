...
function cellsToNodes (boardNode, cell) {
  ...
  node.classList.add('msRow-' + cell.row)
  node.classList.add('msCol-' + cell.col)
...
function markCell (evt) {
  evt.preventDefault();
  if (evt.target.classList.contains("hidden") == true){
...
function displayMessage (msg, id) {
  document.getElementById(id || 'message').innerHTML = '<h1>' + msg + '</h1>'
}

function getRow (element) {
  return parseInt(getCoordinate(element, 'msRow'), 10)
}

function getCol (element) {
  return parseInt(getCoordinate(element, 'msCol'), 10)
}
...
function revealMines (evt) {
  makeArray(document.getElementsByClassName('mine'))
    .forEach(function (element) {
      element.classList.remove('hidden')
      if (element.classList.contains("marked") && element.classList.contains('mine')){
...
function getNodeByCoordinates (row, col) {
  var rowClass = 'msRow-' + row
  var colClass = 'msCol-' + col
